...
$TextBar
Get-FormattedText ""
Get-FormattedText "Powershell script to update software for Switch SD Cards"
Get-FormattedText ""
$TextBar
$TextBar
Get-FormattedText ""
Get-FormattedText "https://www.github.com/HansonGrace/"
$TextBar
""
}
...
function Generate-MainMenuSelections($MainMenuFlags, $FailMessage) {
    ...
    ""
    "Enter letter to toggle updates."
...
while($MainMenuLoop) {
    ...
    elseif($Selection -eq 'c') {
        if($MainMenuFlags -eq 0) {
            $FailMessage = "Must one or more custom firmware components to update."
        }
        else {
            $MainMenuLoop = $false
        }
...
while($ConfigMenuLoop) {
    ...
    elseif($Selection -eq 'f') {
        ...
        $FailMessage = " '$Selection' is not recognized."
    }
}
...
while($DriveMenuLoop) {
    ...
    "For Windows, this will simply be the drive,e.x. D:\"
    "For Linux, this will be a mount path,e.x. /mnt/sd/"
    ""
    $DrivePath = Read-Host -Prompt "Path to SD Card"
    if($DrivePath -eq "") {
        $FailMessage = "Enter the path to your SD card"
    }
    elseif(-not (Test-Path $DrivePath)) {
        $FailMessage = "Could not find path '$DrivePath'"
    }
    else {
        $DriveMenuLoop = $false
    }
